...
export default function ChangePasswordScreen() {
  ...
  const [lockRemainingTime, setLockRemainingTime] = useState<string | null>(
    null,
  );
  ...
  const handleChangePassword = async () => {
    ...
    if (newPassword !== confirmPassword) {
      setErrorMessage(
        "Naya password aur confirm password match nahi kar rahe.",
      );
      return;
    }
    ...
    try {
      ...
      if (
        ...
        if (newAttempts >= 3) {
          ...
          setErrorMessage(
            "Too many failed attempts! Feature locked for 24 hours.",
          );
        } else {
          await updateDoc(doc(db, "users", user.uid), {
            failedPasswordAttempts: newAttempts,
          });
          setFailedAttempts(newAttempts);
          setErrorMessage(
            `Galat password! ${3 - newAttempts} attempts remaining.`,
          );
        }
      } else if (error.code === "auth/requires-recent-login") {
        setErrorMessage(
          "Security issue: Ek baar logout karke wapas login karo, phir try karna.",
        );
      } else {
        setErrorMessage(
          "Password update fail ho gaya. Shayad tumne Google se login kiya tha?",
        );
      }
    } finally {
      setLoading(false);
      ...
      >
        ...
        >
          <View
            style={[
              styles.lockIconBg,
              isLocked && { backgroundColor: "#fef2f2" },
            ]}
          >
            <Ionicons
              name={isLocked ? "lock-closed" : "shield-checkmark"}
              size={40}
              color={isLocked ? "#ef4444" : "#4f46e5"}
            />
          </View>
          <Text style={styles.titleText}>
            {isLocked ? "Access Locked" : "Change Password"}
          </Text>
        ...
        {!isLocked && (
          ...
          >
            ...
            <View
              style={[
                styles.inputWrapper,
                errorMessage?.includes("Current") && { borderColor: "#ef4444" },
              ]}
            >
              <Ionicons
                name="key-outline"
            ...
            <View
              style={[
                styles.inputWrapper,
                errorMessage?.includes("match") && { borderColor: "#ef4444" },
              ]}
            >
              <Ionicons
