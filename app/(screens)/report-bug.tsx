...
export default function ReportBugScreen() {
  ...
  const handleSubmitBug = async () => {
    if (!title.trim() || !description.trim()) {
      Alert.alert(
        "Hold on!",
        "Please provide both a title and description for the bug.",
      );
      return;
    }
    ...
    try {
      ...
      Alert.alert(
        "Bug Reported 🐛",
        "Thanks for helping us improve Eduxity! Our team will look into this.",
      );
      router.back();
    } catch (error) {
      ...
      Alert.alert(
        "Error",
        "Could not submit bug report. Check your network connection.",
      );
    } finally {
      setLoading(false);
    }
          ...
          >
            ...
            <View style={{ flex: 1, marginLeft: 15 }}>
              ...
              <Text style={styles.bannerSub}>
                Let us know what's broken so we can squash the bug and improve
                the app!
              </Text>
            </View>
          </Animated.View>
            ...
            >
              {BUG_TYPES.map((type) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.typePill,
                    bugType === type && styles.activeTypePill,
                  ]}
                  onPress={() => {
                    setBugType(type);
                ...
                >
                  <Text
                    style={[
                      styles.typeText,
                      bugType === type && styles.activeTypeText,
                    ]}
                  >
                    {type}
                  </Text>
