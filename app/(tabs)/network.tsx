...
import { 
  ...
  StatusBar, TextInput, ActivityIndicator, Alert, Platform, Dimensions
} from 'react-native';
...
export default function NetworkScreen() {
  ...
  const calculateMatchMatrix = (user: any, me: any) => {
    ...
    if (user.friendIds && me.friendIds) {
      ...
      if (mutualFriends.length > 0) {
        ...
        matchReasons.push({ reason: `🤝 ${mutualFriends.length} Mutual Connection${mutualFriends.length > 1 ? 's' : ''}`, weight: mutualFriends.length * 25, color: '#f43f5e' }); 
      }
    }
    // Tier 1: Core
    if (user.targetExam && me.targetExam && user.targetExam === me.targetExam) { score += 40; matchReasons.push({ reason: 'Same Target Exam', weight: 40, color: '#2563eb' }); }
    if (user.class && me.class && user.class === me.class) { score += 20; matchReasons.push({ reason: 'Same Class', weight: 20, color: '#8b5cf6' }); }
    // Tier 2: Synergy
    const synergyMatch = user.strongSubjects?.some((s: string) => me.weakSubjects?.includes(s));
    if (synergyMatch) { score += 35; matchReasons.push({ reason: 'Can help you study', weight: 35, color: '#10b981' }); }
    // Tier 3: Logistics
    ...
    // Tier 4: Interests
    if (user.city && me.city && user.city.toLowerCase() === me.city.toLowerCase()) { score += 10; matchReasons.push({ reason: 'From your City', weight: 10, color: '#3b82f6' }); }

    matchReasons.sort((a, b) => b.weight - a.weight);
    return { score, primaryReason: matchReasons.length > 0 ? matchReasons[0].reason : null, badgeColor: matchReasons.length > 0 ? matchReasons[0].color : '#64748b' };
  };
  ...
  const handleSendRequest = async (receiver: any) => {
    ...
    try {
      ...
      await addDoc(collection(db, 'connections'), {
        ...
        receiverAvatar: receiver.photoURL || receiver.profilePic || DEFAULT_AVATAR,
        status: 'pending',
  ...
  // ==========================================
  const renderHeader = () => (
    <View style={[styles.headerContainer, { paddingTop: Platform.OS === 'ios' ? insets.top + 30 : 0 }]}>
      ...
      <View style={styles.headerTop}>
        ...
        <TouchableOpacity style={styles.headerIconBtn} onPress={() => router.push('/search-users')}>
      ...
      <View style={styles.tabContainer}>
        {(['recommended', 'requests', 'friends'] as const).map((tab) => (
          <TouchableOpacity key={tab} style={[styles.tabBtn, activeTab === tab && styles.activeTabBtn]} onPress={() => { setActiveTab(tab); Haptics.selectionAsync(); }}>
  ...
  const renderRecommendedCard = ({ item, index }: any) => (
    <Animated.View entering={FadeInUp.delay(index * 100)} layout={LinearTransition.springify()} style={styles.cardPremium}>
      <TouchableOpacity 
        style={styles.cardInfoCenter} 
        activeOpacity={0.7}
        onPress={() => router.push({ pathname: '/user/[id]', params: { id: item.uid || item.id } })}
      >
        <Image source={{ uri: item.photoURL || item.profilePic || item.avatar || DEFAULT_AVATAR }} style={styles.avatarLarge} />
        ...
        {item.matchReason && (
          <View style={[styles.algoBadge, { backgroundColor: item.badgeColor + '15', borderColor: item.badgeColor + '30' }]}>
      ...
      <TouchableOpacity style={styles.actionBtnFull} onPress={() => handleSendRequest(item)} activeOpacity={0.8}>
        <LinearGradient colors={['#2563EB', '#4F46E5']} style={styles.btnGradientFull}>
  ...
  const renderRequestCard = ({ item, index }: any) => {
    ...
    return (
      <Animated.View entering={FadeInUp.delay(index * 100)} layout={LinearTransition.springify()} style={styles.listCard}>
        <TouchableOpacity style={{ flexDirection: 'row', alignItems: 'center', flex: 1 }} onPress={() => router.push({ pathname: '/user/[id]', params: { id: sId } })}>
          <Image source={{ uri: item.senderAvatar || DEFAULT_AVATAR }} style={styles.avatarMedium} />
          <View style={{ marginLeft: 12, flex: 1 }}>
            <Text style={styles.userNameList} numberOfLines={1}>{item.senderName}</Text>
        ...
        <View style={styles.requestActionRow}>
          ...
          <TouchableOpacity style={styles.iconBtnAccept} onPress={() => handleAcceptRequest(item.id, sId)}>
  ...
  const renderFriendCard = ({ item, index }: any) => {
    ...
    return (
      <Animated.View entering={FadeInUp.delay(index * 50)} layout={LinearTransition.springify()} style={styles.listCard}>
        <TouchableOpacity style={{ flexDirection: 'row', alignItems: 'center', flex: 1 }} onPress={() => router.push({ pathname: '/user/[id]', params: { id: friendId } })}>
          <View>
            <Image source={{ uri: friendAvatar || DEFAULT_AVATAR }} style={styles.avatarMedium} />
            <View style={styles.onlineDot} />
          </View>
          <View style={{ marginLeft: 12, flex: 1 }}>
            <Text style={styles.userNameList} numberOfLines={1}>{friendName}</Text>
            <Text style={[styles.userSubtitleList, {color: '#10b981'}]}>Connected</Text>
          </View>
        </TouchableOpacity>
          ...
          <>
            {activeTab === 'recommended' && (
              <FlatList
                ...
                contentContainerStyle={{ paddingTop: Platform.OS === 'ios' ? 210 : 200, paddingBottom: 150 }}
            ...
            {activeTab === 'requests' && (
              <FlatList
                ...
                contentContainerStyle={{ paddingTop: Platform.OS === 'ios' ? 210 : 200, paddingBottom: 150, paddingHorizontal: 15 }}
                showsVerticalScrollIndicator={false}
...
const styles = StyleSheet.create({
  ...
  // 🍏 Glass Header
  headerContainer: { position: 'absolute', top: 0, width: '100%', zIndex: 50, borderBottomWidth: 1, borderColor: 'rgba(226,232,240,0.6)', paddingBottom: 15 },
  ...
  headerIconBtn: { width: 44, height: 44, borderRadius: 22, backgroundColor: '#eef2ff', justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: '#e2e8f0' },

  searchWrapper: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#f1f5f9', marginHorizontal: 20, borderRadius: 24, height: 48, borderWidth: 1, borderColor: '#e2e8f0', marginBottom: 15 },
  searchInput: { flex: 1, marginLeft: 8, fontSize: 15, color: '#0f172a', fontWeight: '500', height: '100%' },

  tabContainer: { flexDirection: 'row', backgroundColor: '#f1f5f9', borderRadius: 20, padding: 4, marginHorizontal: 20, borderWidth: 1, borderColor: '#e2e8f0' },
  tabBtn: { flex: 1, paddingVertical: 12, alignItems: 'center', borderRadius: 16, position: 'relative' },
  activeTabBtn: { backgroundColor: '#fff', shadowColor: '#000', shadowOffset: {width: 0, height: 2}, shadowOpacity: 0.05, shadowRadius: 5, elevation: 2 },
  tabText: { fontSize: 13, fontWeight: '700', color: '#64748b' },
  activeTabText: { color: '#0f172a', fontWeight: '900' },
  badgeDot: { position: 'absolute', top: 8, right: 12, width: 8, height: 8, backgroundColor: '#ef4444', borderRadius: 4 },

  // Matchmaking Banner
  matchBannerContainer: { marginBottom: 20, borderRadius: 24, overflow: 'hidden', shadowColor: '#ec4899', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.2, shadowRadius: 10, elevation: 5 },
  ...
  matchBannerAction: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', paddingHorizontal: 14, paddingVertical: 10, borderRadius: 20, elevation: 2 },
  matchBannerActionText: { color: '#ec4899', fontWeight: '900', fontSize: 13, marginRight: 4 },

  // Recommended Grid Cards
  cardPremium: { width: '48%', backgroundColor: '#fff', borderRadius: 24, padding: 15, marginBottom: 15, alignItems: 'center', shadowColor: '#0f172a', shadowOffset: {width: 0, height: 4}, shadowOpacity: 0.03, shadowRadius: 10, elevation: 2, borderWidth: 1, borderColor: '#f1f5f9' },
  cardInfoCenter: { alignItems: 'center', marginBottom: 15, width: '100%' },
  avatarLarge: { width: 66, height: 66, borderRadius: 33, backgroundColor: '#f1f5f9', marginBottom: 12, borderWidth: 1, borderColor: '#e2e8f0' },
  userNameLarge: { fontSize: 15, fontWeight: '800', color: '#0f172a', textAlign: 'center', letterSpacing: -0.2 },
  userSubtitle: { fontSize: 11, color: '#64748b', fontWeight: '600', marginTop: 4, textAlign: 'center' },
  
  algoBadge: { paddingHorizontal: 10, paddingVertical: 5, borderRadius: 10, marginTop: 10, borderWidth: 1 },
  algoBadgeText: { fontSize: 10, fontWeight: '800' },

  actionBtnFull: { width: '100%', borderRadius: 14, overflow: 'hidden' },
  btnGradientFull: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 12 },
  btnTextFull: { color: '#fff', fontSize: 13, fontWeight: '800', marginLeft: 6 },

  // List Cards (Requests & Friends)
  listCard: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#fff', padding: 15, borderRadius: 20, marginBottom: 12, shadowColor: '#0f172a', shadowOffset: {width: 0, height: 2}, shadowOpacity: 0.03, shadowRadius: 5, elevation: 1, borderWidth: 1, borderColor: '#f1f5f9' },
  avatarMedium: { width: 52, height: 52, borderRadius: 26, backgroundColor: '#f1f5f9', borderWidth: 1, borderColor: '#e2e8f0' },
  ...
  userNameList: { fontSize: 16, fontWeight: '800', color: '#0f172a', marginBottom: 3, letterSpacing: -0.2 },
  userSubtitleList: { fontSize: 13, color: '#64748b', fontWeight: '500' },

  requestActionRow: { flexDirection: 'row', gap: 10 },
  iconBtnReject: { width: 44, height: 44, borderRadius: 22, backgroundColor: '#fef2f2', justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: '#fee2e2' },
  iconBtnAccept: { width: 44, height: 44, borderRadius: 22, backgroundColor: '#10b981', justifyContent: 'center', alignItems: 'center', shadowColor: '#10b981', shadowOffset: {width:0, height:2}, shadowOpacity: 0.3, shadowRadius: 4, elevation: 2 },
  
  chatIconBtn: { width: 44, height: 44, borderRadius: 22, backgroundColor: '#eef2ff', justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: '#e0e7ff' },
