        ...
        <View style={[styles.barsWrapper, { height: chartHeight }]}>
          {data.map((item, index) => {
            const barHeightPercentage =
              maxValue === 0 ? 0 : (item.count / maxValue) * 100;
            const barHeight = useSharedValue(0);

            useEffect(() => {
              barHeight.value = withDelay(
                index * 100,
                withTiming(barHeightPercentage, { duration: 1000 }),
              );
            }, [data]);
  ...
  const fetchAllData = async () => {
    try {
      ...
      try {
        ...
        const fetchedReports: RecentReport[] = recentReportsData.docs.map(
          (doc) => {
            const data = doc.data();
            return {
              id: doc.id,
  ...
  const generateGrowthChartData = async () => {
    ...
    try {
      ...
      growthDocs.forEach((doc) => {
        ...
        if (joinedAt) {
          const dayStr = joinedAt.toLocaleDateString("en-US", {
            weekday: "short",
          });
      ...
      setMaxChartValue(
        maxCount > 10 ? maxCount + Math.ceil(maxCount * 0.2) : 10,
      );
      setChartData(days);
    } catch (error) {
      setChartData([
      ...
      >
        ...
        {loading ? (
          ...
          <Animated.View entering={FadeInDown.delay(100).duration(400)}>
            {chartData.length > 0 && (
              <AnalyticsChart data={chartData} maxValue={maxChartValue} />
            )}
          </Animated.View>
        )}
            ...
            {loading ? (
              ...
              <View style={styles.emptyStateBox}>
                <Ionicons
                  ...
                  color="#10b981"
                />
                <Text style={styles.emptyTextSuccess}>
