...
export default function AdminAIPostsScreen() {
    ...
    return allPosts.filter((post) => {
      ...
      if (selectedClass !== "ALL") {
        ...
        if (
          !postClass
            .toLowerCase()
            .includes(selectedClass.toLowerCase().replace("class ", ""))
        )
          return false;
      }
      ...
      if (selectedDiff !== "ALL") {
        ...
        if (!postDiff.toLowerCase().includes(selectedDiff.toLowerCase()))
          return false;
      }
      ...
      if (searchQuery.trim() !== "") {
        ...
        const safeContentString = JSON.stringify(
          post.content || {},
        ).toLowerCase();
        const topicString = (post.topic || "").toLowerCase();

        if (
          !topicString.includes(queryText) &&
          !safeContentString.includes(queryText)
        )
          return false;
      }
  ...
  const renderDetailedContent = (type: string, content: any) => {
    ...
    switch (type) {
        ...
        return (
          <View style={styles.contentBlock}>
            <View style={styles.qaBox}>
              <Text style={styles.qaLabel}>FRONT (Q):</Text>
              <Text style={styles.contentText}>{content.front}</Text>
            </View>
            <View
              style={[
                styles.qaBox,
            ...
            >
              <Text style={[styles.qaLabel, { color: "#16a34a" }]}>
                BACK (A):
              </Text>
              <Text style={styles.contentText}>{content.back}</Text>
            </View>
          </View>
        );
      case "quiz_mcq":
        return (
          <View style={styles.contentBlock}>
            <Text style={styles.questionText}>Q: {content.question}</Text>
            <View style={styles.optionsList}>
              {content.options?.map((opt: string, idx: number) => {
                const isCorrect = idx === content.correctAnswerIndex;
                return (
                  <View
                    key={idx}
                    style={[
                      styles.adminOptionBtn,
                      isCorrect && styles.adminOptionCorrect,
                    ]}
                  >
                    <Text
                      style={[
                        styles.adminOptionText,
                        isCorrect && { color: "#16a34a", fontWeight: "800" },
                      ]}
                    >
          ...
          <View style={styles.contentBlock}>
            ...
            >
              ...
              <View
                style={[
                  styles.tfBadge,
                  content.isTrue ? styles.tfTrue : styles.tfFalse,
                ]}
              >
                <Text style={styles.tfBadgeText}>
      ...
      case "mini_game_match":
        return (
          <View style={styles.contentBlock}>
            <Text style={styles.qaLabel}>Match Pairs:</Text>
            {content.pairs?.map((pair: any, idx: number) => (
              <View key={idx} style={styles.pairRow}>
                ...
                <View style={styles.defBox}>
                  <Text style={styles.defText}>{pair.definition}</Text>
                </View>
              </View>
            ))}
          </View>
      ...
      default:
        return <Text style={styles.errorText}>Unknown Format</Text>;
    }
  };
  ...
  const renderPostItem = ({ item, index }: { item: any; index: number }) => {
    ...
    if (item.type === "quiz_mcq") {
      badgeColor = "#dcfce7";
      textColor = "#15803d";
    }
    if (item.type === "quiz_tf") {
      badgeColor = "#fef3c7";
      textColor = "#b45309";
    }
    if (item.type === "mini_game_match") {
      badgeColor = "#e0e7ff";
      textColor = "#4338ca";
    }
    ...
    return (
      ...
      >
        ...
        <View style={styles.contentWrapper}>
          {renderDetailedContent(item.type, item.content)}
        </View>
        <View style={styles.cardFooter}>
          <View style={{ flexDirection: "row", gap: 10 }}>
            <Text style={styles.metaText}>
              📚 {item.userClass || "Class NA"}
            </Text>
            <Text style={styles.metaText}>
  ...
  return (
    <SafeAreaView style={styles.container} edges={["top"]}>
      ...
      <View style={styles.header}>
        ...
        <View>
          <Text style={styles.headerTitle}>God Mode: Data Hub</Text>
      ...
      {showFilters && (
        <Animated.View
          ...
          style={styles.advancedFiltersDrawer}
        >
          ...
          >
            {FILTER_DIFFICULTIES.map((diff) => (
              <TouchableOpacity
                ...
                style={[
                  styles.chip,
                  selectedDiff === diff && styles.chipActive,
                ]}
              ...
              >
                <Text
                  style={[
                    styles.chipText,
                    selectedDiff === diff && styles.chipTextActive,
                  ]}
                >
                  {diff}
                </Text>
          ...
          >
            {FILTER_TYPES.map((type) => (
              <TouchableOpacity
                ...
                style={[
                  styles.chip,
                  selectedType === type.id && styles.chipActive,
                ]}
              ...
              >
                <Text
                  style={[
                    styles.chipText,
                    selectedType === type.id && styles.chipTextActive,
                  ]}
                >
                  {type.label}
                </Text>
          ...
          ListEmptyComponent={
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptyText}>
                No data matches these exact filters.
              </Text>
            </View>
          }
        />
