...
import { 
  collection, query, orderBy, getDocs, doc, deleteDoc, updateDoc, limit 
...
export default function GlobalPostManager() {
  ...
  const [activeFilter, setActiveFilter] = useState<'Recent' | 'Trending' | 'Boosted'>('Recent');
  ...
  const fetchAllPosts = async () => {
    ...
    try {
      ...
      const fetchedPosts: PostData[] = snapshot.docs.map(d => {
        ...
        const likes = typeof data.likesCount === 'number' ? data.likesCount : (data.likes?.length || 0);
        const comments = typeof data.commentsCount === 'number' ? data.commentsCount : (data.comments?.length || 0);
        
        return {
          id: d.id,
          authorName: data.authorName || data.userName || 'Unknown User',
          authorAvatar: data.authorAvatar || data.userAvatar || DEFAULT_AVATAR,
          text: data.text || data.content || data.caption || 'No text content. (Media/Image)',
          likesCount: likes,
  ...
  const toggleBoostPost = async (postId: string, currentStatus: boolean) => {
    ...
    const proceed = async () => {
      ...
      try {
        await updateDoc(doc(db, 'posts', postId), { isBoosted: !currentStatus });
        
        // Update UI Instantly
        setPosts(prev => prev.map(p => p.id === postId ? { ...p, isBoosted: !currentStatus } : p));
    ...
    else if (!isWeb) Alert.alert(action, msg, [{ text: "Cancel", style: "cancel" }, { text: "Confirm", style: "default", onPress: proceed }]);
  };
  ...
  const deletePost = async (postId: string) => {
    ...
    if (isWeb && window.confirm("Permanently delete this post from the platform? This cannot be undone.")) proceed();
    else if (!isWeb) Alert.alert("Delete Post", "Permanently delete this post?", [{ text: "Cancel", style: "cancel" }, { text: "Delete", style: "destructive", onPress: proceed }]);
  };
    ...
    return (
      <View style={[styles.postCard, item.isBoosted && styles.boostedCard]}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              ...
              {item.isBoosted && (
                <View style={styles.boostBadge}>
                  <Ionicons name="rocket" size={10} color="#fff" style={{marginRight: 2}} />
        ...
        <View style={styles.actionRow}>
          <TouchableOpacity 
            style={[styles.actionBtn, item.isBoosted ? styles.unboostBtn : styles.boostBtn]} 
            onPress={() => toggleBoostPost(item.id, item.isBoosted)}
            disabled={isProcessing}
          >
            {isProcessing ? <ActivityIndicator size="small" color={item.isBoosted ? "#f59e0b" : "#fff"} /> : (
              <>
                <Ionicons name="rocket" size={16} color={item.isBoosted ? "#f59e0b" : "#fff"} />
                <Text style={item.isBoosted ? styles.unboostBtnText : styles.boostBtnText}>
    ...
    <View style={styles.container}>
      ...
      <View style={styles.filterContainer}>
        {['Recent', 'Trending', 'Boosted'].map((filter) => (
          <TouchableOpacity 
            ...
            onPress={() => { if(!isWeb) Haptics.selectionAsync(); setActiveFilter(filter as any); }}
          >
      ...
      {loading ? (
        <View style={styles.loaderCenter}>
          ...
          <Text style={{marginTop: 10, color: '#64748b', fontWeight: '600'}}>Fetching Platform Feed...</Text>
        </View>
      ) : (
        <FlatList 
          data={filteredPosts}
...
const styles = StyleSheet.create({
  ...
  filterContainer: { flexDirection: 'row', backgroundColor: '#fff', paddingHorizontal: 20, borderBottomWidth: 1, borderColor: '#e2e8f0' },
  filterTab: { flex: 1, alignItems: 'center', paddingVertical: 15, borderBottomWidth: 2, borderColor: 'transparent' },
  ...
  emptyText: { marginTop: 15, color: '#94a3b8', fontSize: 14, fontWeight: '500' },

  // POST CARD
  postCard: { backgroundColor: '#fff', padding: 15, borderRadius: 16, marginBottom: 15, borderWidth: 1, borderColor: '#e2e8f0', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.02, shadowRadius: 4, elevation: 1 },
  boostedCard: { borderColor: '#fcd34d', backgroundColor: '#fffbeb', borderWidth: 2 },
  
  cardHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: 12 },
  avatar: { width: 40, height: 40, borderRadius: 20, backgroundColor: '#e2e8f0', marginRight: 12 },
  authorName: { fontSize: 15, fontWeight: '800', color: '#0f172a', marginRight: 8 },
  timeText: { fontSize: 11, color: '#94a3b8', fontWeight: '600', marginTop: 2 },
  
  boostBadge: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#f59e0b', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 6 },
  boostBadgeText: { color: '#fff', fontSize: 9, fontWeight: '900', letterSpacing: 0.5 },

  postText: { fontSize: 14, color: '#334155', lineHeight: 22, fontWeight: '500', marginBottom: 15 },

  statsRow: { flexDirection: 'row', gap: 15, backgroundColor: '#f8fafc', padding: 10, borderRadius: 10, marginBottom: 15, borderWidth: 1, borderColor: '#f1f5f9' },
  statItem: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  statText: { fontSize: 11, fontWeight: '700', color: '#64748b' },

  actionRow: { flexDirection: 'row', gap: 10 },
  
  boostBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: '#f59e0b', paddingVertical: 12, borderRadius: 10, gap: 8 },
  boostBtnText: { color: '#fff', fontSize: 13, fontWeight: '800' },
  
  unboostBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: '#fef3c7', borderWidth: 1, borderColor: '#fde68a', paddingVertical: 12, borderRadius: 10, gap: 8 },
  unboostBtnText: { color: '#f59e0b', fontSize: 13, fontWeight: '800' },

  deleteBtn: { alignItems: 'center', justifyContent: 'center', backgroundColor: '#fef2f2', paddingHorizontal: 15, borderRadius: 10, borderWidth: 1, borderColor: '#fecaca' }
});