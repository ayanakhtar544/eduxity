...
import { Audio } from 'expo-av'; // 👈 THE LIFESAVER AUDIO PACKAGE
...
// ==========================================
// (No features removed, kept exactly as requested)
const ChatBubble = ({ message, isMe, groupId, onOpenTest }: any) => {
  ...
  const handleAnswerSubmit = async (answerValue: any) => {
    ...
    try {
      ...
      await updateDoc(msgRef, {
        [`responses.${currentUid}`]: answerValue
      });
    } catch (e) {
      console.log("Answer submit fail hua:", e);
    }
  };

  // 🚀 TYPE 1: THE EXCLUSIVE TEST BUBBLE
  if (message.type === 'test') {
    ...
    return (
      <Animated.View entering={FadeInUp.duration(400).springify()} layout={Layout.springify()} style={[styles.messageWrapper, isMe ? styles.messageWrapperMe : styles.messageWrapperOther]}>
        {!isMe && <Image source={{ uri: message.senderAvatar || `https://ui-avatars.com/api/?name=${message.senderName}` }} style={styles.senderAvatar} />}
        <View style={[styles.bubble, isMe ? styles.bubbleMe : styles.bubbleOther, { width: '85%', padding: 0, overflow: 'hidden' }]}>
          ...
          <LinearGradient colors={['#4f46e5', '#3730a3']} style={{ padding: 15, alignItems: 'center' }}>
            ...
            <Text style={{ color: '#c7d2fe', fontSize: 12, fontWeight: '600', marginTop: 2 }}>
              {message.questions?.length} Questions • {message.duration} Minutes
            </Text>
          </LinearGradient>
          ...
          <View style={{ padding: 15, backgroundColor: isMe ? '#2563eb' : '#fff' }}>
            ...
            {hasTakenTest ? (
              <View style={{ backgroundColor: isMe ? '#1e3a8a' : '#f8fafc', padding: 15, borderRadius: 12, alignItems: 'center', borderWidth: 1, borderColor: isMe ? '#1e40af' : '#e2e8f0' }}>
                ...
                <Text style={{ fontSize: 32, fontWeight: '900', color: isMe ? '#fff' : '#10b981' }}>
                  {testResult.score} <Text style={{ fontSize: 16, color: isMe ? '#bfdbfe' : '#94a3b8' }}>/ {message.questions?.length}</Text>
                </Text>
              </View>
            ) : (
  ...
  // 🚀 TYPE 2: MCQ BUBBLE WITH ANALYTICS
  if (message.type === 'mcq') {
    let correctVotes = 0;
      ...
      <Animated.View entering={FadeInUp.duration(400).springify()} layout={Layout.springify()} style={[styles.messageWrapper, isMe ? styles.messageWrapperMe : styles.messageWrapperOther]}>
        ...
        <View style={[styles.bubble, isMe ? styles.bubbleMe : styles.bubbleOther, { width: '85%' }]}>
          ...
          {message.options?.map((opt: string, index: number) => {
            ...
            if (hasAnswered) {
              let barColor = '#e2e8f0'; 
              if (isCorrect) barColor = '#d1fae5'; 
              else if (isSelected && !isCorrect) barColor = '#fee2e2'; 
              ...
              return (
                <View key={index} style={[styles.mcqResultOption, isCorrect ? styles.mcqCorrectBorder : (isSelected ? styles.mcqWrongBorder : {})]}>
                  ...
                  <View style={styles.mcqResultContent}>
                    ...
                    {isCorrect && <Ionicons name="checkmark-circle" size={20} color="#10b981" style={{marginLeft: 5, zIndex: 10}} />}
                    {isSelected && !isCorrect && <Ionicons name="close-circle" size={20} color="#ef4444" style={{marginLeft: 5, zIndex: 10}} />}
                  </View>
                </View>
              );
            } else {
...
export default function ChatScreen() {
  ...
  // ==========================================
  // 🔄 EFFECTS (LIFECYCLE & AUDIO FIX)
  // ==========================================
  useEffect(() => {
    // 🚨 AUDIO FIX INIT: App ko batata hai ki loudspeaker zinda rakhna hai
    const setupAudio = async () => {
      try {
        await Audio.setAudioModeAsync({
          ...
          playThroughEarpieceAndroid: false, // Loudspeaker Mode
        });
      } catch (e) {
  ...
  const handleAddTodo = async () => {
    ...
    try {
      ...
      setNewTodoText('');
    } catch (e) { Alert.alert("Error", "Task add nahi hua."); }
  };
  ...
  const toggleTodo = async (todoId: string, currentCompletedBy: string[]) => {
    ...
    if (updatedCompletedBy.includes(uid)) updatedCompletedBy = updatedCompletedBy.filter(id => id !== uid);
    else updatedCompletedBy.push(uid);

    try { await updateDoc(doc(db, 'groups', id as string, 'todos', todoId), { completedBy: updatedCompletedBy }); } 
  ...
  const handleSendQuestion = async () => {
    ...
    if (qType === 'mcq') {
      ...
      if (validOptions.length < 2) { Alert.alert("Error", "MCQ mein kam se kam 2 options hone chahiye bhai!"); return; }
    }
    try {
      await addDoc(collection(db, 'groups', id as string, 'messages'), {
        type: qType,
        ...
        responses: {}, 
        senderId: auth.currentUser.uid,
        senderName: auth.currentUser.displayName || 'Eduxity Leader',
        senderAvatar: auth.currentUser.photoURL || `https://ui-avatars.com/api/?name=${auth.currentUser.displayName}`,
        createdAt: serverTimestamp(),
      });
      setShowQuestionModal(false); setQText(''); setMcqOptions(['', '', '', '']);
    } catch (error) { Alert.alert("Error", "Question bhejne mein dikkat aayi."); }
  };
  ...
  const handleSendTest = async () => {
    ...
    try {
      await addDoc(collection(db, 'groups', id as string, 'messages'), {
        ...
        duration: parseInt(testDuration) || 15,
    ...
    } catch (error) { Alert.alert("Error", "Test create nahi hua."); }
  };
  ...
  // 📞 CALL / VOICE LOUNGE PERMISSION ENGINE
  const toggleVoiceLounge = async () => {
    ...
    try {
      if (isInLounge) {
        ...
        // 🚨 JOIN LOUNGE: Request Permission First
        const permissionResponse = await Audio.requestPermissionsAsync();
        
        if (permissionResponse.granted) {
          // Re-affirm Audio mode when joining
          await Audio.setAudioModeAsync({
            allowsRecordingIOS: true,
            playsInSilentModeIOS: true,
            staysActiveInBackground: true,
            shouldDuckAndroid: true,
            playThroughEarpieceAndroid: false, // Loudspeaker Active
          });
          ...
          Alert.alert("Permission Denied", "Mic ki permission ke bina Voice Lounge join nahi kar sakte!");
        }
      }
    } catch (e) { console.log("Lounge Error:", e); }
  };
  ...
  const toggleMic = async () => {
    ...
    try {
      const updatedUsers = activeVoiceUsers.map((u: any) => 
        u.uid === uid ? { ...u, isMuted: newMutedState } : u
      );
      await updateDoc(doc(db, 'groups', id as string), { activeVoice: updatedUsers });
    } catch (e) { console.log("Mic toggle error:", e); }
  };
  ...
  if (isBlocked) {
    return (
      <SafeAreaView style={[styles.container, { justifyContent: 'center', alignItems: 'center' }]}>
        ...
        <Text style={{ fontSize: 15, color: '#64748b', textAlign: 'center', paddingHorizontal: 40, marginTop: 10 }}>
          The leaders of this group have permanently blocked you. You cannot access this chat anymore.
        </Text>
        <TouchableOpacity style={{ marginTop: 30, backgroundColor: '#f1f5f9', padding: 15, borderRadius: 12 }} onPress={() => router.replace('/(tabs)/explore')}>
          <Text style={{ fontWeight: '700', color: '#1e293b' }}>Go Back</Text>
        </TouchableOpacity>
      </SafeAreaView>
    );
  }
      ...
      <View style={styles.headerContainer}>
        ...
        <View style={styles.headerContent}>
          ...
          <TouchableOpacity style={styles.headerTextContainer} onPress={() => router.push(`/chat/info/${id}`)}>
            ...
            <Text style={styles.groupStatus}>Tap for Group Info <Ionicons name="chevron-forward" size={12} color="#bfdbfe" /></Text>
          </TouchableOpacity>
          {/* Header Call Button explicitly joins Lounge */}
          {!isInLounge && (
        ...
        <View style={styles.loungeContainer}>
          <View style={styles.loungeHeader}>
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              ...
              <Text style={styles.loungeTitle}>Voice Lounge • {activeVoiceUsers.length} active</Text>
            </View>
            {!isInLounge ? (
              <TouchableOpacity style={styles.joinLoungeBtn} onPress={toggleVoiceLounge}>
                <Text style={styles.joinLoungeText}>Join</Text>
              </TouchableOpacity>
            ) : (
              <View style={{ flexDirection: 'row' }}>
                <TouchableOpacity style={[styles.micBtn, isMicMuted && {backgroundColor: '#fee2e2'}]} onPress={toggleMic}>
                  <Ionicons name={isMicMuted ? "mic-off" : "mic"} size={18} color={isMicMuted ? "#ef4444" : "#1e293b"} />
                </TouchableOpacity>
                <TouchableOpacity style={styles.leaveLoungeBtn} onPress={toggleVoiceLounge}>
            ...
            {activeVoiceUsers.map((user: any, index: number) => (
              <View key={index} style={styles.loungeUser}>
                <View style={[styles.loungeAvatarWrapper, !user.isMuted && styles.speakingRing]}>
                  ...
                  {user.isMuted && (
                    <View style={styles.mutedBadge}><Ionicons name="mic-off" size={10} color="#fff" /></View>
                  )}
                </View>
      ...
      <KeyboardAvoidingView style={styles.chatArea} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={Platform.OS === 'ios' ? 90 : 0}>
        {loading ? <View style={styles.loaderContainer}><ActivityIndicator size="large" color="#2563eb" /></View> : (
          <FlatList
            ...
            renderItem={({ item }) => <ChatBubble message={item} isMe={item.senderId === auth.currentUser?.uid} groupId={id} onOpenTest={startTest} />}
          />
        )}
                ...
                return (
                  <View style={styles.todoItem}>
                    <TouchableOpacity style={styles.todoRow} onPress={() => toggleTodo(item.id, item.completedBy)}>
                      <Ionicons name={isChecked ? "checkbox" : "square-outline"} size={22} color={isChecked ? "#10b981" : "#94a3b8"} />
                      <Text style={[styles.todoText, isChecked && styles.todoTextDone]}>{item.task}</Text>
                    </TouchableOpacity>
        ...
        {/* --- INPUT AREA --- */}
        {isMuted ? (
          <View style={[styles.inputContainer, { justifyContent: 'center', paddingVertical: 20, backgroundColor: '#fef2f2' }]}>
            <Ionicons name="volume-mute" size={20} color="#ef4444" style={{ marginRight: 8 }} />
            <Text style={{ color: '#ef4444', fontWeight: '700' }}>A Leader has disabled your chat.</Text>
          </View>
        ) : (
          <View style={styles.inputContainer}>
            <TouchableOpacity style={{ marginRight: 10, padding: 5 }} onPress={() => setShowActionMenu(true)}>
              <Ionicons name="add-circle" size={28} color="#94a3b8" />
            </TouchableOpacity>
            <TextInput 
              style={styles.textInput} placeholder="Message..." placeholderTextColor="#94a3b8" 
        ...
        <TouchableOpacity style={{flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end'}} onPress={() => setShowActionMenu(false)}>
          <View style={{ backgroundColor: '#fff', padding: 20, borderTopLeftRadius: 24, borderTopRightRadius: 24 }}>
            ...
            <TouchableOpacity style={styles.actionMenuBtn} onPress={() => { setShowActionMenu(false); setShowQuestionModal(true); }}>
              ...
              <View>
                <Text style={styles.actionMenuTitle}>Poll / Question</Text>
                <Text style={styles.actionMenuSub}>Ask an MCQ or subjective doubt</Text>
              </View>
            </TouchableOpacity>
            <TouchableOpacity style={styles.actionMenuBtn} onPress={() => { setShowActionMenu(false); setShowTestCreator(true); }}>
              ...
              <View>
                <Text style={styles.actionMenuTitle}>Live Test</Text>
                <Text style={styles.actionMenuSub}>Create a timed test with auto-submit</Text>
              </View>
            </TouchableOpacity>
          </View>
        </TouchableOpacity>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{flex: 1, backgroundColor: '#f8fafc'}}>
          ...
          <ScrollView style={{ padding: 20 }}>
            <View style={{ flexDirection: 'row', marginBottom: 20 }}>
              <TouchableOpacity style={[styles.typeBtn, qType === 'mcq' && styles.typeBtnActive]} onPress={() => setQType('mcq')}>
                <Text style={[styles.typeBtnText, qType === 'mcq' && {color: '#fff'}]}>MCQ</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.typeBtn, qType === 'subjective' && styles.typeBtnActive]} onPress={() => setQType('subjective')}>
                <Text style={[styles.typeBtnText, qType === 'subjective' && {color: '#fff'}]}>Written Answer</Text>
              </TouchableOpacity>
            </View>
            ...
            {qType === 'mcq' && (
              <View>
                <Text style={{fontWeight: '700', marginBottom: 8, color: '#1e293b'}}>Options & Correct Answer:</Text>
                {mcqOptions.map((opt, idx) => (
                  <View key={idx} style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 10 }}>
                    <TouchableOpacity onPress={() => setCorrectOptionIdx(idx)} style={{marginRight: 10}}>
                      <Ionicons name={correctOptionIdx === idx ? "radio-button-on" : "radio-button-off"} size={24} color={correctOptionIdx === idx ? "#10b981" : "#94a3b8"} />
                    </TouchableOpacity>
              ...
              <View key={qIndex} style={{ backgroundColor: '#fff', padding: 15, borderRadius: 12, marginBottom: 15, elevation: 1 }}>
                ...
                {q.options.map((opt, oIndex) => (
                  <View key={oIndex} style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
                    <TouchableOpacity onPress={() => { const newQ = [...testQuestions]; newQ[qIndex].correct = oIndex; setTestQuestions(newQ); }} style={{marginRight: 10}}>
                      <Ionicons name={q.correct === oIndex ? "radio-button-on" : "radio-button-off"} size={22} color={q.correct === oIndex ? "#10b981" : "#94a3b8"} />
                    </TouchableOpacity>
      ...
      <Modal visible={activeTest !== null} animationType="slide">
        <SafeAreaView style={{ flex: 1, backgroundColor: '#f8fafc' }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 20, backgroundColor: '#fff', borderBottomWidth: 1, borderColor: '#e2e8f0' }}>
            ...
            <View style={{ backgroundColor: '#fee2e2', paddingHorizontal: 15, paddingVertical: 8, borderRadius: 20, flexDirection: 'row', alignItems: 'center' }}>
              <Ionicons name="timer" size={18} color="#ef4444" style={{ marginRight: 5 }} />
              <Text style={{ color: '#ef4444', fontWeight: '900', fontSize: 16 }}>{Math.floor(timeLeft / 60)}:{('0' + (timeLeft % 60)).slice(-2)}</Text>
            </View>
          </View>
          <ScrollView style={{ padding: 20 }}>
            {activeTest?.questions.map((q: any, qIndex: number) => (
              <View key={qIndex} style={{ backgroundColor: '#fff', padding: 20, borderRadius: 16, marginBottom: 20, elevation: 2 }}>
                ...
                {q.options.map((opt: string, oIndex: number) => (
                  <TouchableOpacity key={oIndex} style={[styles.mcqOption, testAnswers[qIndex] === oIndex && styles.mcqSelected]} onPress={() => { const newAns = [...testAnswers]; newAns[qIndex] = oIndex; setTestAnswers(newAns); }}>
                    <Text style={styles.mcqOptionText}>{opt}</Text>
                  </TouchableOpacity>
                ))}
              </View>
