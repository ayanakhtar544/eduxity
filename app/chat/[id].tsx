...
// 🛑 CHILD COMPONENTS (Removed LiveTestModal)
...
export default function MVPAdvancedChatEngine() {
  ...
  useEffect(() => {
    ...
    const unsubGroup = onSnapshot(doc(db, "chats", chatIdStr), async (docSnap) => {
      if (docSnap.exists()) {
        const data = docSnap.data();
        setGroupInfo(data);

        if (chatIdStr.includes("_") || data.isGroup === false) {
          let targetUid = (data.members || []).find((memberUid: string) => memberUid !== uid);
    ...
    const unsubMessages = onSnapshot(
      query(collection(db, "chats", chatIdStr, "messages"), orderBy("createdAt", "asc")),
      (snapshot) => {
        const fetched = snapshot.docs.map((d) => ({ id: d.id, ...d.data() }));
        setMessages(fetched);
        const newest = fetched[fetched.length - 1] as any;
        if (newest?.id && newest.id !== lastMessageIdRef.current && newest.senderId !== auth.currentUser?.uid) {
          Toast.show({
            type: "success",
            text1: "New message",
        ...
        const hws = fetched.filter(
          (m) => m.type === "homework_bucket" && (!m.submissions || !m.submissions[auth.currentUser!.uid]),
        );
  ...
  const sendMessage = async () => {
    ...
    const updates: any = {
      lastMessage: { id: added.id, text: textToSend, type: "text", senderName: auth.currentUser.displayName || "User" },
    ...
    if (membersArray.length > 0) {
      ...
      membersArray.forEach((memberUid: string) => {
        if (memberUid !== myUid) updates[`unreadCount.${memberUid}`] = increment(1);
      });
    }
  ...
  const handleReaction = async (emoji: string) => {
    ...
    if (Platform.OS !== "web") Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    await updateDoc(doc(db, "chats", chatIdStr, "messages", selectedMessage.id), { [`reactions.${auth.currentUser.uid}`]: emoji });
    setSelectedMessage(null);
  };

  const handlePinMessage = async () => {
    if (!selectedMessage || !isLeader) return;
    await updateDoc(doc(db, "chats", chatIdStr), { pinnedMessageId: selectedMessage.id });
    setSelectedMessage(null);
  ...
  const syncParentDocument = async (msgObj: any) => {
    ...
    if (membersArray.length > 0) {
      updates.members = membersArray;
      membersArray.forEach((m: string) => { if (m !== myUid) updates[`unreadCount.${m}`] = increment(1); });
    }
  ...
  const handleCreateTodo = async () => {
    ...
    await addDoc(collection(db, "groups", chatIdStr, "todos"), {
  ...
  const handleCreateStudyRoom = async () => {
    ...
    const docRef = await addDoc(collection(db, "chats", chatIdStr, "messages"), {
      type: "study_session",
    ...
    return (
      <SafeAreaView style={[styles.container, { justifyContent: "center", alignItems: "center" }]}>
        ...
        <TouchableOpacity style={styles.goBackBtn} onPress={() => router.back()}>
          <Text style={{ fontWeight: "800", color: "#fff" }}>Return Home</Text>
        </TouchableOpacity>
      </SafeAreaView>
    );
  }
    ...
    return (
      <View style={[styles.dockWrapper, { bottom: Platform.OS === "ios" ? insets.bottom + 10 : 15 }]}>
        ...
        <View style={styles.dockContainer}>
          {Platform.OS === "ios" ? (
            <BlurView intensity={85} tint="light" style={styles.glassBackground} />
          ) : (
            <View style={[styles.glassBackground, { backgroundColor: "rgba(255,255,255,0.95)" }]} />
          )}
  ...
  return (
    <SafeAreaView style={styles.container} edges={["left", "right"]}>
      ...
      <View style={[styles.floatingHeaderContainer, { top: Platform.OS === "ios" ? insets.top + 10 : StatusBar.currentHeight! + 10 }]}>
        ...
        <View style={styles.headerTop}>
          ...
          <TouchableOpacity style={styles.headerInfo} activeOpacity={0.7} onPress={() => router.push(`/chat/info/${chatIdStr}`)}>
            <View>
              <Image
                source={{ uri: otherUser?.photoURL || groupInfo?.groupImage || groupInfo?.image || groupInfo?.photoURL || groupInfo?.avatar || DEFAULT_AVATAR }}
          ...
          <View style={styles.headerRightActions}>
            {!!groupInfo?.unreadCount?.[auth.currentUser?.uid || ""] && (
              <View style={styles.unreadBadge}>
                <Text style={styles.unreadBadgeText}>{groupInfo.unreadCount[auth.currentUser?.uid || ""]}</Text>
              </View>
            )}
      ...
      <View style={styles.bannerArea}>
        {pendingHomeworkCount > 0 && activeTab === "Tasks" && (
          <Animated.View entering={SlideInDown} exiting={SlideOutDown} style={styles.homeworkBanner}>
            ...
            <Text style={styles.homeworkBannerText}>{pendingHomeworkCount} Homework submission due!</Text>
          </Animated.View>
        )}
        {pinnedMessage && (
          <Animated.View entering={FadeInDown} exiting={FadeOutDown} style={styles.pinnedStrip}>
            <View style={styles.pinnedLeft}>
              ...
              <View style={{ marginLeft: 8, flex: 1 }}>
                ...
                <Text style={styles.pinnedText} numberOfLines={1}>{pinnedMessage.title || pinnedMessage.text}</Text>
              </View>
            </View>
            <TouchableOpacity onPress={() => setPinnedMessage(null)} style={{ padding: 4 }}>
      ...
      <KeyboardAvoidingView style={styles.chatArea} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        {loading ? (
          <View style={styles.loaderContainer}><ActivityIndicator size="large" color="#4f46e5" /></View>
        ) : (
          <FlatList
            ref={flatListRef}
            data={messagesWithDates}
            keyExtractor={(item, index) => item.id || index.toString()}
            contentContainerStyle={{ paddingTop: Platform.OS === "ios" ? 150 : 140, paddingBottom: 150, paddingHorizontal: 15 }}
        ...
        {showJumpToBottom && (
          <Animated.View entering={ZoomIn} exiting={ZoomOut} style={styles.jumpBottomWrapper}>
            <TouchableOpacity style={styles.jumpBottomBtn} onPress={() => flatListRef.current?.scrollToEnd({ animated: true })}>
        ...
        <Modal visible={true} transparent animationType="fade">
          <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setSelectedMessage(null)}>
            <Animated.View entering={FadeInDown.springify()} style={styles.quickActionBox}>
              ...
              <TouchableOpacity style={styles.actionListItem} onPress={() => { setReplyingTo(selectedMessage); setSelectedMessage(null); }}>
                <Ionicons name="arrow-undo-outline" size={22} color="#0f172a" />
                <Text style={styles.actionListText}>Reply</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.actionListItem} onPress={() => setSelectedMessage(null)}>
                <Ionicons name="chatbubbles-outline" size={22} color="#0f172a" />
                <Text style={styles.actionListText}>Reply in Thread</Text>
              </TouchableOpacity>
              {isLeader && (
                <TouchableOpacity style={styles.actionListItem} onPress={handlePinMessage}>
                  <Ionicons name="pin-outline" size={22} color="#0f172a" />
                  <Text style={styles.actionListText}>Pin Message</Text>
                </TouchableOpacity>
              )}
            </Animated.View>
          </TouchableOpacity>
        ...
        <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setShowActionMenu(false)}>
          <Animated.View entering={SlideInDown.springify().damping(15)} exiting={SlideOutDown} style={styles.glassSheet}>
            ...
            <View style={styles.toolsGrid}>
              <TouchableOpacity style={styles.toolCard} onPress={() => { setShowActionMenu(false); setShowQuestionModal(true); }}>
                <View style={[styles.toolIcon, { backgroundColor: "#eef2ff" }]}>
                  <Ionicons name="stats-chart" size={28} color="#4f46e5" />
                </View>
                <Text style={styles.toolCardTitle}>Poll / MCQ</Text>
              </TouchableOpacity>
              {/* Removed Live Test Feature */}
              <TouchableOpacity style={styles.toolCard} onPress={() => { setShowActionMenu(false); setBucketModalVisible(true); }}>
                <View style={[styles.toolIcon, { backgroundColor: "#fefce8" }]}>
                  <Ionicons name="folder-open" size={28} color="#ca8a04" />
                </View>
                <Text style={styles.toolCardTitle}>Homework</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.toolCard} onPress={() => { setShowActionMenu(false); setTodoModalVisible(true); }}>
                <View style={[styles.toolIcon, { backgroundColor: "#f0fdf4" }]}>
                  <Ionicons name="checkbox" size={28} color="#16a34a" />
                </View>
                <Text style={styles.toolCardTitle}>To-Do Task</Text>
              </TouchableOpacity>
            </View>
          </Animated.View>
        </TouchableOpacity>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1, backgroundColor: "#f8fafc" }}>
          ...
          <View style={styles.modalBody}>
            ...
            <TextInput
              ...
              placeholder="e.g. Read Physics Chapter 5..."
              value={newTodoTask}
              onChangeText={setNewTodoTask}
              maxLength={LIMITS.MAX_TITLE_LENGTH}
            />
            <TouchableOpacity style={styles.primaryActionBtn} onPress={handleCreateTodo}>
      ...
      <Modal visible={bucketModalVisible} animationType="slide" presentationStyle="pageSheet">
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1, backgroundColor: "#f8fafc" }}>
          ...
          <View style={styles.modalBody}>
            ...
            <TextInput
              style={styles.modalInputFull}
              placeholder="e.g. Differentiation Problems"
              value={bucketTitle}
              onChangeText={setBucketTitle}
              maxLength={LIMITS.MAX_TITLE_LENGTH}
            />
            <TouchableOpacity style={styles.primaryActionBtn} onPress={createHomeworkBucket}>
              <Text style={styles.primaryActionText}>Create Bucket</Text>
            </TouchableOpacity>
          </View>
        </KeyboardAvoidingView>
      </Modal>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1, backgroundColor: "#f8fafc" }}>
          ...
          <ScrollView style={styles.modalBody}>
            <View style={styles.pollTabs}>
              <TouchableOpacity style={[styles.pollTab, qType === "mcq" && styles.pollTabActive]} onPress={() => setQType("mcq")}>
                <Text style={[styles.pollTabText, qType === "mcq" && styles.pollTabTextActive]}>MCQ Poll</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.pollTab, qType === "subjective" && styles.pollTabActive]} onPress={() => setQType("subjective")}>
                <Text style={[styles.pollTabText, qType === "subjective" && styles.pollTabTextActive]}>Written Doubt</Text>
              </TouchableOpacity>
            </View>
            <Text style={styles.inputLabel}>QUESTION TEXT</Text>
            <TextInput
              style={[styles.modalInputFull, { minHeight: 100, textAlignVertical: "top" }]}
              ...
              <View>
                ...
                {mcqOptions.map((opt, idx) => (
                  <View key={idx} style={styles.optionRow}>
                    <TouchableOpacity onPress={() => setCorrectOptionIdx(idx)} style={{ marginRight: 12 }}>
                      <Ionicons name={correctOptionIdx === idx ? "checkmark-circle" : "ellipse-outline"} size={28} color={correctOptionIdx === idx ? "#10b981" : "#cbd5e1"} />
                    </TouchableOpacity>
                    <TextInput
                      style={[styles.modalInputFull, { flex: 1, marginBottom: 0 }]}
            ...
            <TouchableOpacity style={[styles.primaryActionBtn, { marginTop: 30, marginBottom: 50 }]} onPress={handleSendQuestion}>
              <Text style={styles.primaryActionText}>Launch Question</Text>
            </TouchableOpacity>
          </ScrollView>
        </KeyboardAvoidingView>
      </Modal>
      ...
      <Modal visible={showStudyModal} animationType="slide" presentationStyle="pageSheet">
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1, backgroundColor: "#f8fafc" }}>
          <View style={{ padding: 25 }}>
            ...
            <TextInput
              style={[styles.modalInputFull, { height: 55, marginBottom: 25 }]}
              placeholder="e.g. 25"
              keyboardType="numeric"
              value={studyDuration}
              onChangeText={setStudyDuration}
            />
            <TouchableOpacity style={styles.primaryActionBtn} onPress={handleCreateStudyRoom}>
              <Text style={styles.primaryActionText}>Start Room</Text>
            </TouchableOpacity>
            <TouchableOpacity style={{ marginTop: 20, alignItems: "center" }} onPress={() => setShowStudyModal(false)}>
...
const styles = StyleSheet.create({
  ...
  unreadBadge: { minWidth: 24, height: 24, borderRadius: 12, backgroundColor: "#ef4444", alignItems: "center", justifyContent: "center", paddingHorizontal: 6 },
