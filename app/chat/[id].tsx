...
export default function MVPAdvancedChatEngine() {
  ...
  const [activeDashboardMessage, setActiveDashboardMessage] =
    useState<any>(null);
    ...
    const unsubMessages = onSnapshot(
      ...
      (snapshot) => {
        ...
        const hws = fetched.filter(
          (m) =>
            m.type === "homework_bucket" &&
            (!m.submissions || !m.submissions[auth.currentUser!.uid]),
        );
  ...
  const filteredMessages = useMemo(() => {
    ...
    if (activeTab === "Doubts")
      filtered = messages.filter(
        (m) => m.type === "subjective" || m.text?.includes("?"),
      );
    if (activeTab === "Resources")
      filtered = messages.filter(
        (m) => m.type === "resource" || m.fileUrl || m.imageUrl,
      );
    if (activeTab === "Tasks")
      filtered = messages.filter((m) =>
        ["test", "homework_bucket", "mcq"].includes(m.type),
      );
    ...
    filteredMessages.forEach((msg) => {
      if (msg.createdAt) {
        ...
        if (dateString === today) dateString = "Today";
          ...
          dateString = msgDateObj.toLocaleDateString("en-US", {
            month: "short",
            day: "numeric",
          });
  ...
  const isLeader =
    ...
    groupInfo?.adminId === auth.currentUser?.uid;
  const isBlocked = groupInfo?.blocked?.includes(auth.currentUser?.uid);
  const typingUids =
    groupInfo?.typing?.filter((uid: string) => uid !== auth.currentUser?.uid) ||
    [];
  const typingText =
    typingUids.length > 0
      ? typingUids.length > 1
  ...
  const handleMentionSelect = (user: any) => {
    ...
    words.pop();
    setInputText(
      words.join(" ") + (words.length > 0 ? " " : "") + `@${user.name} `,
    );
    setShowMentions(false);
    if (Platform.OS !== "web")
  ...
  const sendMessage = async () => {
    ...
    sendBtnScale.value = withSpring(0.7, {}, () => {
  ...
  const handleSendQuestion = async () => {
    ...
    const payload = {
      ...
      text: qText.trim(),
      options:
        qType === "mcq" ? mcqOptions.filter((opt) => opt.trim() !== "") : [],
  ...
  const handleSendTest = async () => {
    if (!testTitle.trim() || testQuestions.length === 0 || !auth.currentUser)
      return;
    const payload = {
      type: "test",
  ...
  const renderAppleDock = () => {
    const displayMentions = mentionList
    ...
    return (
      <View
        style={[
          styles.dockWrapper,
          { bottom: Platform.OS === "ios" ? insets.bottom + 10 : 15 },
        ]}
      >
          ...
          <View style={styles.dockInner}>
            ...
            <Animated.View style={animatedSendStyle}>
              {inputText.trim().length > 0 ? (
                <TouchableOpacity
                  style={[styles.dockSendBtn, styles.dockSendBtnActive]}
                  onPress={sendMessage}
                >
                  <Ionicons
                    name="send"
                ...
                <TouchableOpacity
                  style={[styles.dockSendBtn, styles.dockSendBtnInactive]}
                  onPress={() => setShowTaskDrawer(true)}
                >
      ...
      >
        ...
        >
          <View style={styles.taskDrawerHeader}>
            ...
            <TouchableOpacity
              onPress={() => setShowTaskDrawer(false)}
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            {tasks.map((task) => {
              const iCompleted = task.completedBy?.some(
                ...
                <View key={task.id} style={styles.taskCard}>
                  <TouchableOpacity
                    ...
                    onPress={async () => {
                      ...
                      if (iCompleted) {
                        await updateDoc(ref, {
                          completedBy: task.completedBy.filter(
                            (u: any) => u.uid !== auth.currentUser?.uid,
                          ),
                        });
                      } else {
                        await updateDoc(ref, {
                          completedBy: arrayUnion({
                            ...
                            avatar:
                              auth.currentUser?.photoURL || DEFAULT_AVATAR,
                          }),
                        });
                      }
                    }}
                  >
                    <Ionicons
                      name={iCompleted ? "checkmark-circle" : "ellipse-outline"}
                      size={28}
                      color={iCompleted ? "#10b981" : "#cbd5e1"}
                    />
                  </TouchableOpacity>
                  <View style={{ flex: 1 }}>
                    <Text
                      style={[
                        styles.taskText,
                        iCompleted && styles.taskTextCompleted,
                      ]}
                    >
                      {task.task}
                    </Text>
  ...
  return (
    <SafeAreaView style={styles.container} edges={["left", "right"]}>
      ...
      >
        ...
        <View style={styles.headerTop}>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.7}
            onPress={() => router.push(`/chat/info/${chatIdStr}`)}
          >
            <View>
              <Image
                source={{
            ...
            <View style={{ flex: 1, marginLeft: 10 }}>
              <Text style={styles.headerTitle} numberOfLines={1}>
                {otherUser?.displayName || groupInfo?.name || "Chat"}
              </Text>
      ...
      <View style={styles.bannerArea}>
        {pendingHomeworkCount > 0 && activeTab === "Tasks" && (
          <Animated.View
            ...
            style={styles.homeworkBanner}
          >
            <Ionicons name="alert-circle" size={16} color="#e11d48" />
            <Text style={styles.homeworkBannerText}>
              {pendingHomeworkCount} Homework submission due!
            </Text>
          </Animated.View>
        )}
        {pinnedMessage && (
            ...
            <View style={styles.pinnedLeft}>
              ...
              <View style={{ marginLeft: 8, flex: 1 }}>
                ...
                <Text style={styles.pinnedText} numberOfLines={1}>
                  {pinnedMessage.title || pinnedMessage.text}
                </Text>
              </View>
            </View>
            <TouchableOpacity
      ...
      >
            ...
            onScroll={(e) =>
              setShowJumpToBottom(
                e.nativeEvent.contentOffset.y <
                  e.nativeEvent.contentSize.height - height - 100,
              )
            }
            renderItem={({ item }) => {
        ...
        {showJumpToBottom && (
          <Animated.View
            entering={ZoomIn}
            exiting={ZoomOut}
            style={styles.jumpBottomWrapper}
          >
            <TouchableOpacity
          ...
          >
            ...
            <View style={styles.toolsGrid}>
              ...
              >
                ...
                <Text style={styles.toolCardTitle}>Live Test</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={styles.toolCard}
                onPress={() => {
          ...
          <ScrollView style={styles.modalBody}>
            <View style={styles.pollTabs}>
              <TouchableOpacity
                style={[
                  styles.pollTab,
                  qType === "mcq" && styles.pollTabActive,
                ]}
              ...
              >
                <Text
                  style={[
                    styles.pollTabText,
                    qType === "mcq" && styles.pollTabTextActive,
                  ]}
                >
                  MCQ Poll
                </Text>
              ...
              <TouchableOpacity
                style={[
                  styles.pollTab,
                  qType === "subjective" && styles.pollTabActive,
                ]}
                onPress={() => setQType("subjective")}
              >
                <Text
                  style={[
                    styles.pollTabText,
                    qType === "subjective" && styles.pollTabTextActive,
                  ]}
                >
                  Written Doubt
                </Text>
              ...
              <View>
                ...
                {mcqOptions.map((opt, idx) => (
                  <View key={idx} style={styles.optionRow}>
                    <TouchableOpacity
                      onPress={() => setCorrectOptionIdx(idx)}
                    ...
                    >
                      <Ionicons
                        ...
                        size={28}
                        color={correctOptionIdx === idx ? "#10b981" : "#cbd5e1"}
                      />
                    </TouchableOpacity>
                    <TextInput
                      style={[
      ...
      >
        ...
        >
          ...
          <ScrollView style={styles.modalBody}>
            ...
            <TextInput
              style={styles.modalInputFull}
              placeholder="e.g. Physics Full Mock"
              value={testTitle}
                ...
                <TouchableOpacity
                  style={[
                    styles.modalInputFull,
                    {
                      ...
                      alignItems: "center",
                      backgroundColor: testNtaFormat ? "#ecfdf5" : "#f8fafc",
                      borderColor: testNtaFormat ? "#10b981" : "#e2e8f0",
                    },
                  ]}
                  onPress={() => setTestNtaFormat(!testNtaFormat)}
                >
                  <Text
                    style={{
                      color: testNtaFormat ? "#059669" : "#64748b",
                      fontWeight: "800",
                    }}
                  >
                    {testNtaFormat ? "ENABLED" : "DISABLED"}
                  </Text>
                </TouchableOpacity>
              </View>
            </View>
                ...
                <View style={styles.qcHeader}>
                  ...
                  {testQuestions.length > 1 && (
                    <TouchableOpacity
                      onPress={() =>
                        setTestQuestions(
                          testQuestions.filter((_, i) => i !== qIdx),
                        )
                      }
                    >
                  ...
                  <View key={oIdx} style={styles.optionRow}>
                    ...
                    >
                      <Ionicons
                        ...
                        size={24}
                        color={q.correct === oIdx ? "#4f46e5" : "#cbd5e1"}
                      />
                    </TouchableOpacity>
                    <TextInput
                      style={[
            ...
            >
              <Text style={styles.primaryActionText}>Launch Test Now</Text>
            </TouchableOpacity>
          </ScrollView>
        </KeyboardAvoidingView>
      </Modal>
      ...
      >
        ...
        >
          <View style={{ padding: 25 }}>
            ...
            <Text
              style={{ color: "#64748b", marginBottom: 8, fontWeight: "700" }}
            >
              Focus Duration (Mins)
            </Text>
            <TextInput
            ...
            >
              <Text
                style={{ color: "#64748b", fontWeight: "700", fontSize: 16 }}
              >
                Cancel
              </Text>
            </TouchableOpacity>
      ...
      />
      <ImageViewerModal
        visibleImage={viewerImage}
        setVisibleImage={setViewerImage}
      />
    </SafeAreaView>
  );
}
