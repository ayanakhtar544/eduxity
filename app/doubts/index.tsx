...
const calculateDoubtScore = (doubt: any, userInterests: string[]) => {
  ...
  if (userInterests && userInterests.length > 0) {
    ...
    const allDoubtKeywords = [...doubtTags, ...doubtSubject].map((k) =>
      k.toLowerCase(),
    );

    const matchCount = userInterests.filter((interest) =>
      allDoubtKeywords.includes(interest.toLowerCase()),
    ).length;
    score += matchCount * 25;
  }
  ...
  return (
    <View style={styles.boostContainer}>
      <TouchableOpacity activeOpacity={0.8} onPress={handlePress}>
        <Animated.View
          style={[
            styles.boostIconWrapper,
            animatedStyle,
            isBoosted && styles.boostedBg,
          ]}
        >
          <Ionicons
            name="flash"
            size={22}
            color={isBoosted ? "#fff" : "#94a3b8"}
          />
        </Animated.View>
      </TouchableOpacity>
      <Text style={[styles.boostCount, isBoosted && styles.boostedText]}>
        {count}
      </Text>
    </View>
  );
...
export default function DoubtHubScreen() {
  ...
  const processedFeed = useMemo(() => {
    ...
    filtered.sort(
      (a, b) =>
        calculateDoubtScore(b, userInterests) -
        calculateDoubtScore(a, userInterests),
    );
    return filtered;
      ...
      prev.map((d) => {
        if (d.id === doubtId)
          return {
            ...d,
            boosts: isBoosted
              ? d.boosts.filter((id: string) => id !== uid)
              : [...d.boosts, uid],
          };
        return d;
      }),
  ...
  const handlePostAnswer = async (item: any) => {
    ...
    try {
      ...
      try {
        const reward =
          (await processAction(currentUser.uid, "ANSWER_DOUBT")) ||
          (await processAction(currentUser.uid, "CREATE_POST"));
  ...
  const renderDoubtCard = ({ item, index }: any) => {
    const isBoosted = currentUser
      ? item.boosts?.includes(currentUser.uid)
      : false;
    ...
    return (
      ...
      >
        <View style={styles.leftActionBar}>
          <BoostButton
            isBoosted={isBoosted}
            count={item.boosts?.length || 0}
            onPress={() => handleBoost(item.id, item.boosts || [])}
          />
        </View>
        ...
        <View style={styles.cardContent}>
          <View style={styles.cardHeader}>
            ...
            {item.isSolved ? (
              ...
              <View style={styles.unsolvedBadge}>
                ...
                <Text style={styles.unsolvedText}>Open</Text>
              </View>
            )}
          </View>
          ...
          {item.topAnswer ? (
            <View style={styles.topAnswerBox}>
              <View style={styles.topAnswerHeader}>
                ...
                <Text style={styles.topAnswerLabel}>
                  Top Answer by {item.topAnswer.authorName}
                </Text>
              </View>
              <Text style={styles.topAnswerText} numberOfLines={3}>
                {item.topAnswer.text}
              </Text>
            </View>
          ) : (
          ...
          <View style={styles.cardFooter}>
            <View style={styles.tagsRow}>
              {item.subject && (
                <View style={styles.subjectPillDark}>
                  <Text style={styles.subjectTextDark}>{item.subject}</Text>
                </View>
              )}
            </View>
          ...
          {isExpanded && (
            <Animated.View
              ...
              style={styles.inlineReplyContainer}
            >
  ...
  return (
    <SafeAreaView style={styles.container} edges={["top", "left", "right"]}>
            ...
            ListEmptyComponent={
              <View style={styles.emptyState}>
                ...
                <Text style={styles.emptySub}>
                  Ask a doubt and let the community help you out!
                </Text>
              </View>
            }
          />
      ...
      <TouchableOpacity
        ...
        activeOpacity={0.9}
        onPress={() => router.push("/doubts/ask")}
      >
        <LinearGradient
