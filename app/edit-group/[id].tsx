...
export default function EditGroupScreen() {
    ...
    const fetchGroupData = async () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          const groupAdminId = data.adminId || (data.admins ? data.admins[0] : null);
          if (currentUid === groupAdminId || data.admins?.includes(currentUid)) {
            setIsAdmin(true);
          }
        }
      } catch (error) {
  ...
  if (fetching) return <View style={styles.center}><ActivityIndicator size="large" color="#4f46e5" /></View>;

  const displayImage = logoUri || existingLogo || `https://ui-avatars.com/api/?name=${encodeURIComponent(name)}&background=0f172a&color=fff&size=200&bold=true`;
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>{isAdmin ? 'Group Settings' : 'Group Info'}</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          ...
          <Animated.View entering={FadeInDown.duration(500).springify()} style={styles.logoUploadContainer}>
            <TouchableOpacity activeOpacity={0.8} style={styles.logoPlaceholder} onPress={pickImage} disabled={!isAdmin}>
              <Image source={{ uri: displayImage }} style={styles.previewImage} transition={200} />
              {isAdmin && <View style={styles.editBadge}><Ionicons name="pencil" size={14} color="#fff" /></View>}
            </TouchableOpacity>
            <Text style={styles.hintText}>{isAdmin ? 'Tap to change logo' : 'Group Logo'}</Text>
          </Animated.View>
          ...
          <Animated.View entering={FadeInDown.delay(150).duration(600).springify()} style={styles.formGroup}>
            ...
            <View style={[styles.inputWrapper, !isAdmin && styles.inputDisabled]}>
              <TextInput style={styles.input} value={name} onChangeText={setName} maxLength={30} editable={isAdmin} />
            </View>
          </Animated.View>

          <Animated.View entering={FadeInDown.delay(250).duration(600).springify()} style={styles.formGroup}>
            <Text style={styles.label}>Description</Text>
            <View style={[styles.inputWrapper, { height: 100 }, !isAdmin && styles.inputDisabled]}>
              <TextInput style={[styles.input, styles.textArea]} value={description} onChangeText={setDescription} multiline numberOfLines={3} maxLength={150} editable={isAdmin} />
            </View>
          </Animated.View>
          ...
          <Animated.View entering={FadeInDown.delay(350).duration(600).springify()} style={styles.formGroup}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.categoryRow}>
              {CATEGORIES.map(cat => {
                ...
                return (
                  <TouchableOpacity key={cat.name} disabled={!isAdmin} style={[styles.catPill, isActive && styles.catPillActive, !isAdmin && !isActive && {opacity: 0.5}]} onPress={() => { Haptics.selectionAsync(); setCategory(cat.name); }}>
          ...
          {/* 🔥 DANGER ZONE 🔥 */}
          <Animated.View entering={FadeInDown.delay(450).duration(600).springify()} style={styles.dangerZone}>
            <Text style={styles.dangerTitle}>Danger Zone</Text>
            {isAdmin ? (
              <TouchableOpacity style={styles.deleteGroupBtn} onPress={handleDeleteGroup}>
                <Ionicons name="trash-outline" size={20} color="#ef4444" />
                <Text style={styles.deleteGroupText}>Delete Group Permanently</Text>
              </TouchableOpacity>
            ) : (
      ...
      {isAdmin && (
        <Animated.View entering={SlideInUp.delay(400).springify()} style={styles.footer}>
          <TouchableOpacity activeOpacity={0.9} onPress={handleUpdateGroup} disabled={loading} style={styles.submitBtnWrapper}>
            <LinearGradient colors={['#4f46e5', '#3b82f6']} start={{x:0,y:0}} end={{x:1,y:1}} style={styles.submitGradient}>
              {loading ? <ActivityIndicator color="#fff" size="small" /> : <Text style={styles.submitBtnText}>Save Changes</Text>}
            </LinearGradient>
          </TouchableOpacity>
        </Animated.View>
      )}
...
const styles = StyleSheet.create({
  ...
  inputDisabled: { backgroundColor: '#f1f5f9', borderColor: '#e2e8f0', opacity: 0.8 },
  ...
  // 🔥 DANGER ZONE STYLES
  dangerZone: { marginTop: 15, paddingTop: 25, borderTopWidth: 1, borderColor: '#e2e8f0' },
  dangerTitle: { fontSize: 14, fontWeight: '900', color: '#ef4444', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 15 },
  
  deleteGroupBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: '#fef2f2', paddingVertical: 16, borderRadius: 16, borderWidth: 1, borderColor: '#fecaca' },
  deleteGroupText: { color: '#ef4444', fontSize: 15, fontWeight: '800', marginLeft: 8 },
  
  leaveGroupBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: '#fffbeb', paddingVertical: 16, borderRadius: 16, borderWidth: 1, borderColor: '#fde68a' },
  leaveGroupText: { color: '#f59e0b', fontSize: 15, fontWeight: '800', marginLeft: 8 },
