...
export default function TestAnalysisScreen() {
    ...
    const fetchAnalysis = async () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          if (data.responses) {
            const rawResponses = Object.entries(data.responses).map(
              ([uid, val]: any) => ({
                uid,
                ...val,
              }),
  ...
  const stats = useMemo(() => {
    if (participants.length === 0)
      return { total: 0, average: 0, maxScore: 0, topper: null };

    const total = participants.length;
    const totalScoreSum = participants.reduce(
        ...
        {participants.length === 0 ? (
          <Animated.View entering={FadeInDown} style={styles.emptyState}>
            ...
            <Text style={styles.emptySub}>
              Share the test link with your students to see analytics here.
            </Text>
          </Animated.View>
        ) : (
          <>
            ...
            <View style={styles.statsGrid}>
              ...
              >
                <View
                  style={[styles.statIconBg, { backgroundColor: "#e0e7ff" }]}
                >
              ...
              >
                ...
                <Text style={styles.statValue}>
                  {stats.average}{" "}
                  <Text style={styles.statSmall}>/ {maxPossibleMarks}</Text>
                </Text>
            ...
            {stats.topper && (
              ...
              >
                ...
                >
                  ...
                  <Image
                    source={{ uri: stats.topper.userAvatar || DEFAULT_AVATAR }}
                    style={styles.topperAvatar}
                  />
                  <Text style={styles.topperName}>
                    {stats.topper.userName || "Scholar"}
                  </Text>
                  <View style={styles.topperScoreRow}>
                    <Text style={styles.topperScore}>
                      {stats.topper.score}{" "}
                      <Text
                        style={{ fontSize: 14, color: "#fff", opacity: 0.8 }}
                      >
                        Marks
                      </Text>
                    </Text>
              ...
              {participants.map((p, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Image
                      source={{ uri: p.userAvatar || DEFAULT_AVATAR }}
                    ...
                    <View style={styles.studentInfo}>
                      ...
                      {p.timeTaken && (
                        <Text style={styles.studentTime}>
                          Time: {Math.floor(p.timeTaken / 60)}m{" "}
                          {p.timeTaken % 60}s
                        </Text>
                      )}
                    </View>
