...
const SmartScoreCard = ({
  ...
  const getPerfColor = (val: number) =>
    val >= 75 ? "#10b981" : val >= 40 ? "#f59e0b" : "#ef4444";
    ...
    <LinearGradient colors={["#0f172a", "#1e293b"]} style={styles.heroCard}>
      ...
      <View style={styles.scoreRow}>
        <View
          style={[
            styles.scoreCircle,
            { borderColor: perfColor, shadowColor: perfColor },
          ]}
        >
        ...
        <View style={styles.statsGrid}>
          <View style={styles.statBox}>
            ...
            <Text style={[styles.statValue, { color: perfColor }]}>
              {accuracy}%
            </Text>
          </View>
          <View style={styles.statBox}>
            ...
            <Text style={[styles.statValue, { color: "#38bdf8" }]}>
              {percentile} <Text style={{ fontSize: 12 }}>%ile</Text>
            </Text>
          </View>
          <View
            style={[
          ...
          >
            ...
            <Text style={[styles.statValue, { color: "#fde047" }]}>
              #{rank}
            </Text>
          </View>
        </View>
...
const WhatIfSimulator = ({ currentScore, potentialGain, incorrectQs }: any) => {
  ...
  return (
    <View
      style={[
        styles.card,
        { borderColor: "#8b5cf6", backgroundColor: "#faf5ff" },
      ]}
    >
      ...
      <Text style={styles.simText}>
        ...
        <Text style={{ fontWeight: "900", color: "#ef4444" }}>
          {incorrectQs} wrong answers
        </Text>{" "}
        par negative marking nahi khayi hoti aur unhe sahi kiya hota, toh
        tumhara score kya hota?
      </Text>
      ...
      <View style={styles.insightPillDark}>
        ...
        <Text style={styles.insightPillTxt}>
          Avoiding negatives could boost you by {potentialGain} marks!
        </Text>
      </View>
    </View>
  );
      ...
      <View style={{ flexDirection: "row", gap: 12 }}>
        <View style={styles.mistakeBox}>
          ...
          <Text style={styles.mistakeDesc}>
            Lack of knowledge or wrong formula.
          </Text>
        </View>
        <View
          style={[
        ...
        >
          ...
          <Text style={[styles.mistakeType, { color: "#b91c1c" }]}>
            Silly Errors
          </Text>
          <Text style={[styles.mistakeDesc, { color: "#f87171" }]}>
            Calculation or reading mistakes.
          </Text>
        </View>
      </View>
    </View>
    ...
    <View style={styles.card}>
      ...
      <View style={{ marginTop: 5 }}>
        {Object.keys(sectionStats).map((secId) => {
          ...
          const secPercentage =
            stat.max > 0
              ? Math.max(0, Math.round((stat.scored / stat.max) * 100))
              : 0;
          let barColor =
            secPercentage >= 75
              ? "#10b981"
          ...
          return (
            <View key={secId} style={styles.progressContainer}>
              <View style={styles.progressLabelRow}>
                <Text style={styles.progressSubName} numberOfLines={1}>
                  {stat.name}
                </Text>
                <Text style={styles.progressSubMarks}>
                  {stat.scored} / {stat.max}
                </Text>
              </View>
              <View style={styles.progressBarBg}>
                <View
                  style={[
                    ...
                    { width: `${secPercentage}%`, backgroundColor: barColor },
                  ]}
                />
              </View>
            </View>
...
const TimeIntelligence = () => {
  return (
    <View style={styles.card}>
      ...
      <Text style={styles.timeInsight}>
        ⏱️ You spent 15% of your time on unattempted questions. Don't let ego
        get in the way of skipping hard questions!
      </Text>
    </View>
  );
};
    ...
    <View style={styles.card}>
      ...
      <View style={styles.legendRow}>
        <View style={styles.legendItem}>
          ...
          <Text style={styles.legendTxt}>Correct ({correctQs})</Text>
        </View>
        <View style={styles.legendItem}>
          <View style={[styles.legendDot, { backgroundColor: "#ef4444" }]} />
          <Text style={styles.legendTxt}>Wrong ({incorrectQs})</Text>
        </View>
        <View style={styles.legendItem}>
          <View style={[styles.legendDot, { backgroundColor: "#e2e8f0" }]} />
          <Text style={styles.legendTxt}>Skipped ({unattemptedQs})</Text>
        </View>
      </View>
        ...
        {examData?.questions.map((q: any, idx: number) => {
          ...
          if (
            ...
            if (q.type === "single_mcq")
              ...
              isCorrect =
                String(uAns).trim() === String(q.numericalAnswer).trim();
            else if (q.type === "multi_mcq")
              isCorrect =
                JSON.stringify(q.correctIndices.sort()) ===
...
export default function AdvancedAnalyticsDashboard() {
    ...
    return (
      <SafeAreaView style={styles.center}>
        ...
        <Text style={styles.errorText}>
          No result found. Did you submit the exam?
        </Text>
        <TouchableOpacity
  ...
  let sectionStats: Record<
    string,
    { name: string; max: number; scored: number }
  > = {};
  examData?.subjects?.forEach((sub: any) => {
    sectionStats[sub.id] = { name: sub.name, max: 0, scored: 0 };
  });
    ...
    if (!hasAnswered) {
      ...
      if (q.type === "single_mcq") {
        ...
        const isFullyCorrect =
          ...
          correctAns.every((val: number) => userAns.includes(val));
        const hasIncorrectOption = userAns.some(
          (val: number) => !correctAns.includes(val),
        );
  ...
  const accuracy =
    attemptedQs > 0 ? Math.round((correctQs / attemptedQs) * 100) : 0;

  // Generating Dynamic Rank & Percentile Based on Accuracy (Mock Logic for wow-factor)
  const percentile =
    accuracy > 90 ? 99.8 : accuracy > 70 ? 92.5 : accuracy > 40 ? 75.2 : 35.4;
  const rank =
    accuracy > 90 ? 12 : accuracy > 70 ? 450 : accuracy > 40 ? 2300 : 15400;
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView
        ...
        contentContainerStyle={{ flexGrow: 1, padding: 15, paddingBottom: 100 }}
      >
        {/* WIDGETS ASSEMBLY */}
        <SmartScoreCard
        ...
        <View style={{ flexDirection: "row", gap: 15 }}>
          <View style={{ flex: 1 }}>
            <SectionalRadar sectionStats={sectionStats} />
          </View>
        </View>
        ...
        {Object.keys(sectionStats).length > 0 && (
          <SectionalAnalysis sectionStats={sectionStats} />
        )}
        ...
        <View style={styles.aiCard}>
          <View style={styles.cardHeader}>
            <MaterialCommunityIcons
              ...
              color="#fff"
            />
            <Text style={[styles.cardTitle, { color: "#fff", marginLeft: 8 }]}>
              AI Coach Plan
            </Text>
          </View>
          <View style={styles.aiTaskItem}>
            <Ionicons
              ...
              style={{ marginTop: 6 }}
            />
            <Text style={styles.aiTaskTxt}>
              Your accuracy is{" "}
              <Text style={{ fontWeight: "bold", color: "#fde047" }}>
                {accuracy}%
              </Text>
              . You need to stop guessing answers to reduce negative marking.
            </Text>
          </View>
          <View style={styles.aiTaskItem}>
            <Ionicons
            ...
            />
            <Text style={styles.aiTaskTxt}>
              Focus on Integer Type questions. You left most of them blank.
            </Text>
          </View>
        </View>
      </ScrollView>
      ...
      <View style={styles.footer}>
        <TouchableOpacity
          ...
          onPress={() => alert("Retake Mistakes mode unlocking soon!")}
        >
