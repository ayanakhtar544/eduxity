...
export default function NtaExamEngine() {
  ...
  const [phase, setPhase] = useState<
    "LOADING" | "INSTRUCTIONS" | "PLAYING" | "SUBMITTING"
  >("LOADING");
  ...
  const handleStartTest = async () => {
    try {
      ...
      if (cached) {
        ...
        setGlobalTimeLeft(
          examData?.rules?.globalDuration
            ? examData.rules.globalDuration * 60
            : 10800,
        );
      }
      setPhase("PLAYING");
    ...
    const autosave = setInterval(async () => {
      try {
        await AsyncStorage.setItem(
          cacheKey,
          JSON.stringify({ answers, qStatus, timeLeft: globalTimeLeft }),
        );
      } catch (e) {
  ...
  const executeFinalSubmit = async (reason: string) => {
      ...
      examData.questions.forEach((q: any) => {
        ...
        if (hasAnswered) {
          if (q.type === "single_mcq") {
            ...
            if (String(uAns).trim() === String(q.numericalAnswer).trim())
              rawScore += q.marks || 4;
            else rawScore -= q.negMarks || 1;
    ...
    try {
      ...
      Alert.alert(
        "Error",
        "Upload failed, but your answers are saved locally.",
      );
