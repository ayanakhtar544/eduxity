    ...
    const fetchAllData = async () => {
      try {
        ...
        if (!isOwnProfile && currentUid) {
          const conn1 = await getDoc(
            doc(db, "connections", `${currentUid}_${id}`),
          );
          const conn2 = await getDoc(
            doc(db, "connections", `${id}_${currentUid}`),
          );

          if (conn1.exists()) {
            setConnectionStatus(
              conn1.data().status === "accepted" ? "friends" : "pending",
            );
          } else if (conn2.exists()) {
            setConnectionStatus(
              conn2.data().status === "accepted" ? "friends" : "pending",
            );
          }
        }
      } catch (error) {
  ...
  if (!profileUser)
    return (
      <View style={styles.center}>
        <Text style={styles.errorTxt}>User not found in Eduxity</Text>
      </View>
    );
        ...
        <View style={styles.testCardInner}>
          ...
          <View style={styles.testCardFooter}>
            <Text style={styles.testFooterTxt}>
              <Ionicons name="list" size={12} /> {post.questions?.length || 0}{" "}
              Qs
            </Text>
            <Text style={styles.testFooterTxt}>
              <Ionicons name="time" size={12} />{" "}
              {post.rules?.globalDuration || 0} Min
            </Text>
          </View>
        </View>
      );
      ...
      return (
        <View style={styles.resourceCardInner}>
          <View style={styles.resourceHeaderRow}>
            <Ionicons
              name={post.type === "resource" ? "book" : "layers"}
              size={16}
              color="#ec4899"
            />
            <Text style={styles.resourceBadgeTxt}>
              {post.type === "resource" ? "Notes" : "Flashcard"}
            </Text>
          </View>
      ...
      >
        ...
        >
          ...
          <Text style={styles.username}>
            @{profileUser.username || name.toLowerCase().replace(/\s/g, "")}
          </Text>
          ...
          <View style={styles.academicRow}>
            ...
            >
              ...
              <Text style={[styles.academicText, { color: "#d97706" }]}>
                {exam}
              </Text>
            </View>
          </View>
