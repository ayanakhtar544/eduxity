...
export default function SidebarMenu({ isOpen, onClose, currentUid }: { isOpen: boolean, onClose: () => void, currentUid: string | undefined }) {
    ...
    <Animated.View entering={FadeIn.duration(200)} exiting={FadeOut.duration(200)} style={styles.menuOverlay}>
      ...
      <Animated.View entering={SlideInLeft.duration(250)} exiting={SlideOutLeft.duration(200)} style={styles.menuDrawer}>
        ...
        <ScrollView contentContainerStyle={styles.menuScroll}>
          ...
          <TouchableOpacity style={styles.menuItem} onPress={() => { onClose(); router.push('/matchmaking'); }}><View style={[styles.menuIconBg, { backgroundColor: '#fdf2f8' }]}><Ionicons name="heart" size={20} color="#ec4899" /></View><Text style={styles.menuItemText}>Find Study Partner</Text></TouchableOpacity>
          
          <Text style={styles.menuSectionTitle}>ACCOUNT</Text>
          <TouchableOpacity style={styles.menuItem} onPress={() => { onClose(); router.push(`/user/${currentUid}`); }}><View style={[styles.menuIconBg, { backgroundColor: '#f1f5f9' }]}><Ionicons name="person" size={20} color="#475569" /></View><Text style={styles.menuItemText}>My Profile</Text></TouchableOpacity>
        </ScrollView>
      </Animated.View>
    </Animated.View>
  );
