...
export default function LiveTestModal({ activeTest, onClose, groupId }: any) {
  ...
  const submitTest = async () => {
    ...
    const maxMarks = activeTest.questions.reduce(
    ...
    const reward = await awardXP(
      ...
      earnedXP,
      "Premium Test Submitted",
    );

    if (reward?.leveledUp) {
      Alert.alert(
        "🎉 EXAM SUBMITTED & LEVEL UP!",
        `Score: ${score}/${maxMarks}\nCorrect: ${correctCount} | Wrong: ${wrongCount}\nNew Level: ${reward.newLevel}`,
      );
    } else {
      Alert.alert(
        "Exam Submitted! 📝",
        `Your score is ${score} out of ${maxMarks}.\nCorrect: ${correctCount} | Wrong: ${wrongCount}\nYou earned +${earnedXP} XP!`,
      );
    }
    onClose();
  };
      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: "#f1f5f9" }}>
        ...
        <View style={styles.modalHeader}>
          <View style={{ flex: 1 }}>
            ...
            {activeTest.ntaFormat && (
              <Text style={styles.ntaSub}>NTA Strict Marking Enabled</Text>
            )}
          </View>
          ...
          {activeTest.questions.map((q: any, qIdx: number) => {
            ...
            return (
              <View
                ...
                style={[
                  styles.questionCard,
                  isAttempted && { borderColor: "#c7d2fe" },
                ]}
              >
                ...
                {q.options.map((opt: string, oIdx: number) => {
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      style={[
                        styles.mcqOption,
                        isSelected && styles.mcqOptionActive,
                      ]}
                ...
                {isAttempted && (
                  <TouchableOpacity
                    onPress={() => toggleOption(qIdx, testAnswers[qIdx])}
