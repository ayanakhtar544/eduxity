...
export default function QuestionBubble({ message, groupId, isMe }: any) {
  ...
  const getPercentage = (optIdx: number) => {
    ...
    const votesForOpt = Object.values(responses).filter(
      (v: any) => v === optIdx,
    ).length;
  ...
  return (
    ...
    >
      ...
      <View style={styles.header}>
        ...
        >
          <Ionicons
            name={isMCQ ? "stats-chart" : "help-outline"}
            size={16}
            color={isMe ? "#fff" : "#4f46e5"}
          />
        </View>
        <Text
          style={[
            styles.typeText,
            isMe ? { color: "#e0e7ff" } : { color: "#4f46e5" },
          ]}
        >
        ...
        {hasAnswered && (
          ...
          >
            <Ionicons
              name="checkmark-done"
              size={12}
              color={isMe ? "#a7f3d0" : "#10b981"}
            />
            <Text
              style={[styles.statusBadgeText, isMe ? { color: "#a7f3d0" } : {}]}
            >
              Done
            </Text>
          </View>
      ...
      {isMCQ && (
        <View style={styles.mcqWrapper}>
          {message.options.map((opt: string, idx: number) => {
            ...
            const isWinner =
              message.correctOption !== null &&
              message.correctOption !== undefined
                ? message.correctOption === idx
                : false;
            const didIVoteThis = responses[myUid!] === idx;

            const barStyle = useAnimatedStyle(() => ({
              width: withTiming(`${percent}%`, { duration: 1000 }),
            }));
                  ...
                  <View style={styles.votedContentRow}>
                    ...
                    <Text
                      style={[
                        ...
                        isWinner && { color: isMe ? "#a7f3d0" : "#059669" },
                      ]}
                    >
                      {percent}%
                    </Text>
            ...
            return (
              ...
              >
                <View
                  style={[
                    styles.voteRadio,
                    isMe ? { borderColor: "rgba(255,255,255,0.5)" } : {},
                  ]}
                >
                  {selectedOption === idx && (
                    <View
                      style={[
                        styles.voteRadioInner,
                        isMe ? { backgroundColor: "#fff" } : {},
                      ]}
                    />
                  )}
                </View>
                <Text
                  style={[styles.voteOptText, isMe ? { color: "#fff" } : {}]}
                >
                  {opt}
                </Text>
              </TouchableOpacity>
          ...
          <Text
            style={[
              ...
              isMe ? { color: "#c7d2fe" } : { color: "#94a3b8" },
            ]}
          >
      ...
      {!isMCQ && (
        <View style={styles.subjectiveWrapper}>
          {!hasAnswered ? (
            ...
            >
              <Text
                style={[styles.myAnswerLabel, isMe ? { color: "#a7f3d0" } : {}]}
              >
                ✅ Your Answer
              </Text>
              <Text
                style={[styles.myAnswerText, isMe ? { color: "#fff" } : {}]}
                numberOfLines={4}
              >
                {responses[myUid!].text}
              </Text>
            </Animated.View>
          )}
            ...
            >
              <TouchableOpacity
                style={[
                  ...
                  isMe ? { backgroundColor: "transparent" } : {},
                ]}
                onPress={() => setShowAnswers(!showAnswers)}
              >
                <Text
                  style={[
                    styles.viewAnswersText,
                    isMe ? { color: "#e0e7ff" } : {},
                  ]}
                >
                  ...
                  {Object.values(responses).map((res: any, idx: number) => (
                    ...
                    >
                      <View style={styles.answerItemHeader}>
                        <Image
                          source={{ uri: res.avatar || DEFAULT_AVATAR }}
